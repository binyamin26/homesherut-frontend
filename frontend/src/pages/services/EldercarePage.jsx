...
const EldercarePageClean = () => {
  ...
  const loadProviders = async () => {
    ...
    console.log('🔍 Paramètres:', cleanParams);
     ...
     setError(t('services.eldercare.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
...
useEffect(() => {
  ...
  <h1 className="service-title" style={{ textAlign: 'center', margin: '0 auto' }}>{t('services.eldercare.pageTitle')}</h1>
  <div style={{ 
  position: 'absolute', 
  right: '0',
  width: '120px',
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
            <div className="loading-text">{t('services.eldercare.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
                <strong>{resultsCount}</strong> {t('services.eldercare.found')}
  ...
  {!locationFilter.neighborhood && locationFilter.city && <span> {t('common.in')} {locationFilter.city}</span>}
          ...
          <p>{t('services.eldercare.loading')}</p>
            </div>
          ) : error ? (
  ...
  />
            ...
            <div className="no-results">
             <h3>{t('services.eldercare.noResults')}</h3>
...
export default EldercarePageClean;