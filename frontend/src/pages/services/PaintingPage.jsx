...
import { Paintbrush, CheckCircle, Star, Phone, Shield, Clock, MessageCircle } from 'lucide-react';
...
const PaintingPage = () => {
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      if (response.success) {
       ...
       setError(t('services.painting.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  <h1 className="service-title" style={{ textAlign: 'center', margin: '0 auto' }}>{t('services.painting.pageTitle')}</h1>
   <div style={{ 
  position: 'absolute', 
  right: '0',
  width: '120px',
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
             <div className="loading-text">{t('services.painting.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
                 <strong>{resultsCount}</strong> {t('services.painting.found')}
  ...
  {!locationFilter.neighborhood && locationFilter.city && <span> {t('common.in')} {locationFilter.city}</span>}
         ...
         <p>{t('services.painting.loading')}</p>
            </div>
          ) : error ? (
  ...
  />
            ...
            <h3>{t('services.painting.noResults')}</h3>
