...
const ContractorPage = () => {
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      if (response.success) {
       ...
       setError(t('services.contractor.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  <h1 className="service-title" style={{ textAlign: 'center', margin: '0 auto' }}>{t('services.contractor.pageTitle')}</h1>
 <div style={{ 
  position: 'absolute', 
  right: '0',
  width: '120px',
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
              <div className="loading-text">{t('services.contractor.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
                <strong>{resultsCount}</strong> {t('services.contractor.found')}
  ...
  {!locationFilter.neighborhood && locationFilter.city && <span> {t('common.in')} {locationFilter.city}</span>}
          ...
          {loading ? (
            <div className="providers-loading">
             ...
             <p>{t('services.contractor.loading')}</p>
            </div>
          ) : error ? (
  ...
  />
            ...
            <h3>{t('services.contractor.noResults')}</h3>
