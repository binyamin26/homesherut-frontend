...
import { Layers, CheckCircle, Star, Phone, Shield, Clock, MessageCircle } from 'lucide-react';
...
const DrywallPage = () => {
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      setError(t('services.drywall.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  <h1 className="service-title" style={{ textAlign: 'center', margin: '0 auto' }}>{t('services.drywall.pageTitle')}</h1>
 <div style={{ 
  position: 'absolute', 
  right: '0',
  width: '120px',
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
             <div className="loading-text">{t('services.drywall.searching')}</div>
              ) : error ? (
  ...
  {!locationFilter.neighborhood && locationFilter.city && <span> {t('common.in')} {locationFilter.city}</span>}
          ...
          {loading ? (
           ...
           <p>{t('services.drywall.loading')}</p>
            </div>
          ) : error ? (
  ...
  />
            ...
            <h3>{t('services.drywall.noResults')}</h3>
