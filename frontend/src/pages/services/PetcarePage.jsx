...
import { PawPrint, CheckCircle, Star, Phone, Heart, Shield, MessageCircle } from 'lucide-react';
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      if (response.success) {
       ...
       setError(t('services.petcare.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
...
useEffect(() => {
  ...
  <h1 className="service-title" style={{ textAlign: 'center', margin: '0 auto' }}>{t('services.petcare.pageTitle')}</h1>
  <div style={{ 
  position: 'absolute', 
  right: '0',
  width: '120px',
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
             <div className="loading-text">{t('services.petcare.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
              <strong>{resultsCount}</strong> {t('services.petcare.found')}
  ...
  {!locationFilter.neighborhood && locationFilter.city && <span> {t('common.in')} {locationFilter.city}</span>}
          ...
          {loading ? (
           ...
           <p>{t('services.petcare.loading')}</p>
            </div>
          ) : error ? (
  ...
  />
            ...
            <div className="no-results">
             <h3>{t('services.petcare.noResults')}</h3>
...
export default PetcarePageClean;