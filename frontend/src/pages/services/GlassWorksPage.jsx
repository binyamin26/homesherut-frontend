...
const GlassWorksPage = () => {
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      setError(t('services.glass_works.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  <h1 className="service-title" style={{ textAlign: 'center', margin: '0 auto' }}>{t('services.glass_works.pageTitle')}</h1>
    <div style={{ 
  position: 'absolute', 
  right: '0',
  width: '120px',
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
            <div className="loading-text">{t('services.glass_works.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
                <strong>{resultsCount}</strong> {t('services.glass_works.found')}
  ...
  {!locationFilter.neighborhood && locationFilter.city && <span> {t('common.in')} {locationFilter.city}</span>}
          ...
          {loading ? (
            <div className="providers-loading">
              ...
              <p>{t('services.glass_works.loading')}</p>
            </div>
          ) : error ? (
  ...
  />
            ...
            <div className="no-results">
             <h3>{t('services.glass_works.noResults')}</h3>
