...
const ElectricianPage = () => {
  ...
  const loadProviders = async () => {
   ...
   setError(t('services.electrician.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  <h1 className="service-title" style={{ textAlign: 'center', margin: '0 auto' }}>{t('services.electrician.pageTitle')}</h1>
   <div style={{ 
  position: 'absolute', 
  right: '0',
  width: '120px',
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
             <div className="loading-text">{t('services.electrician.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
               <strong>{resultsCount}</strong> {t('services.electrician.found')}
  ...
  {!locationFilter.neighborhood && locationFilter.city && <span> {t('common.in')} {locationFilter.city}</span>}
          ...
          {loading ? (
            ...
            <p>{t('services.electrician.loading')}</p>
            </div>
          ) : error ? (
  ...
  />
            ...
            <div className="no-results">
              <h3>{t('services.electrician.noResults')}</h3>
