...
import { Droplets, CheckCircle, Star, Phone, Shield, Clock, MessageCircle } from 'lucide-react';
...
const WaterproofingPage = () => {
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      setError(t('services.waterproofing.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  <h1 className="service-title" style={{ textAlign: 'center', margin: '0 auto' }}>{t('services.waterproofing.pageTitle')}</h1>
   <div style={{ 
  position: 'absolute', 
  right: '0',
  width: '120px',
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
              <div className="loading-text">{t('services.waterproofing.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
              <strong>{resultsCount}</strong> {t('services.waterproofing.found')}
  ...
  {!locationFilter.neighborhood && locationFilter.city && <span> {t('common.in')} {locationFilter.city}</span>}
       ...
       <p>{t('services.waterproofing.loading')}</p>
            </div>
          ) : error ? (
  ...
  />
            ...
            <div className="no-results">
             <h3>{t('services.waterproofing.noResults')}</h3>
